...
parser.add_argument("--policy-episode-timeslots", default=50000, type=int)    
parser.add_argument("--policy-timeslots", default=50000, type=int)   
...
#                            average_delay_benchmark.append([sum(ar)/len(ar) if len(ar) > 0 else 0 for ar in env.processed_packets_t_benchmark])
                ...
                if(sim % args.episode_timeslots == 0):
                    ...
                    if(sim > 0):
                        ...
                        env.packets_t = [[] for i in range(env.N)] 
                        env.processed_packets_t = [[] for i in range(env.N)]
                        p_strategy_all = []
                        user_strategy_all = []
                        all_delay_time = []
                        all_reward = []
                        ...
                        if args.compare == True:
                            ...
                            np_save_path_benchmark = '%s/%sep%d.npz'%(folder_name,'benchmark',int(sim / args.episode_timeslots))
                            np.savez(np_save_path_benchmark,  
                                      all_delay_time_benchmark,
                                      all_reward_benchmark,                     
                                      p_strategy_all_benchmark)
                            env.packets_benchmark = [[] for i in range(env.N)]
                            env.packets_t_benchmark = [[] for i in range(env.N)] 
                            env.processed_packets_t_benchmark = [[] for i in range(env.N)]
                            all_delay_time_benchmark = []
                            all_reward_benchmark = []
                            p_strategy_all_benchmark  = []
